	    ...
	    <div class="modal-dialog">
	        <div class="modal-content">
	            <div class="modal-header">
	                ...
	                <h4 class="modal-title" id="myModalLabel">您要关注此股票?</h4>
	            </div>
	            <div class="modal-body">
	            	1.该公司未来是否还会有更坏的消息?<br/>
	            	2.该公司是否会倒闭?其股票是否会退市?<br/>
	            	3.该公司是否是某个行业或区域的前三?<br/>
	            </div>
	            <div class="modal-footer">
	            	<button id="confirm" type="button" class="btn btn-primary">关注</button>
	                <button id="cancel" type="button" class="btn btn-default" data-dismiss="modal">不关注</button>
	            </div>
	        </div><!-- /.modal-content -->
	    </div><!-- /.modal -->
	</div>
		...
		<ul>
			...
			<li class="row" id="<fmt:formatDate value="${stockNews.pubDate}" pattern="yyyy-MM-dd HH:mm" />,${stockNews.id}">
			  <div class="col-xs-3" style="margin-top:5px;padding-right:5px">
			    <div class="row">
			      <div class="col-xs-12" style="padding-right:5px"><a onclick="selectStock('${stockNews.stockCode}')">
				...
				<div class="row">
				  ...
				  <c:if test="${stockNews.topNum>2}">${stockNews.topNum}次</c:if>
				  </div>
				</div>
			  </div>
			   <div class="col-xs-8" style="margin-top:5px;">
							<a href="${stockNews.newsLink}">${stockNews.title}&nbsp;</a>(<fmt:formatDate value="${stockNews.pubDate}" pattern="yyyy/MM/dd HH:mm" />)
			  </div>
			</li>
			</c:forEach>			
		</ul>
							...
							success : function(data) {
								...
								$.each(refreshNewsList, function(index, item) {
									...
									if($.inArray(id,allIds)==-1){
										...
										var topNum = refreshNewsList[index].topNum;
										...
										if(topNum>0){
											favorNews = favorNews+topNum+"次";
										}
										favorNews = favorNews+"</div></div></div><div class='col-xs-8' style='margin-top:5px'>"+"<a href='"+newsLink+"'>"+title+"</a>";
							...
							success : function(data) {
								...
								$.each(moreNewsList, function(index, item) {
									...
									if($.inArray(id,allIds)==-1){
										...
										var topNum = moreNewsList[index].topNum;
